...
export class ModalComponent {
  ...
  async submitRequest() {
    ...
    if (!this.validateEmail(this.email)) {
      ...
      this.API.failedSnackbar('Please enter a valid email.');
      return;
    }
    if (this.birthdate && new Date(this.birthdate) > new Date(this.maxBirthdate)) {
      this.inputErrors.birthdate = true;
      this.API.failedSnackbar('Please enter a valid birthdate.');
      return;
    }

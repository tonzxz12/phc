...
export class NewclassComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data) {
      this.isEditMode = true; // Set edit mode if data exists
      this.className = this.data.className;
      this.classCode = this.data.classCode;
      this.originalClassCode = this.data.classCode;
      this.classID = this.data.classID;
      this.course = this.data.courseID;
  ...
  async manageClass() {
    try {
      ...
      // Only check for existing class code if:
      // 1. Creating new class OR
      // 2. Editing and the class code has changed
      if (!this.isEditMode || (this.isEditMode && this.classCode !== this.originalClassCode)) {
      ...
      this.API.justSnackbar(`${this.isEditMode ? 'Updating' : 'Creating'} class ....`);
      ...
      if (!this.isEditMode) {
        ...
        const newClassData = await firstValueFrom(
          this.API.getClassFromCode(this.course, this.classCode)
        );

        if (newClassData.success && newClassData.output.length) {
          await firstValueFrom(this.API.createSpeechlab(newClassData.output[0].id));
          this.API.successSnackbar('Class created successfully');
          this.closeModal('update');
        } else {
          throw new Error('Failed to get class data after creation');
        }
      } else {
      ...
      this.API.failedSnackbar('Error managing class. Please try again.');
      console.error('Error in manageClass:', error);
    }
  }

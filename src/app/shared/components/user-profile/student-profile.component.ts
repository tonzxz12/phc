...
export class StudentProfileComponent implements OnInit {
  private originalPersonalInfo: any;
  ...
  ngOnInit(): void {
    ...
    // Store original personal info for cancel
  ...
  toggleEdit() {
    ...
    if (!this.editing) {
      ...
      // Update originalUser after save
      this.originalUser = { ...this.user };
    }
  ...
  savePersonalInfo() {
    this.API.updateStudentInfo(this.user).subscribe(
      (response) => {
        ...
        this.API.updateLocalUserData(JSON.stringify(this.user));
        // Update originalPersonalInfo after save
        this.originalPersonalInfo = {
          birthdate: this.user.birthdate,
          gender: this.user.gender,
          nationality: this.user.nationality,
  ...
  cancelEditPersonalInfo() {
    // Revert user data to original personal info
    this.user.birthdate = this.originalPersonalInfo.birthdate;
    this.user.gender = this.originalPersonalInfo.gender;
    this.user.nationality = this.originalPersonalInfo.nationality;
    this.user.address = this.originalPersonalInfo.address;
    this.user.email = this.originalPersonalInfo.email;
    this.editingPersonalInfo = false;
  }
  
  toggleEditPassword() {
    if (this.editingPassword) {
      // Only check when saving (when editingPassword is true)
      console.log('Input:', this.currentPasswordInput);
      console.log('User Password:', this.user.password);

      if (this.currentPasswordInput !== this.user.password) {
        this.API.failedSnackbar('Current password is incorrect.');
        return;
      }
      ...
      if (this.newPassword === this.user.password) {
        this.API.failedSnackbar('New password must be different from the current password.');
        return;
      }
      // Only now update the user's password
  ...
  // Toggle editing state (open or close the form)
